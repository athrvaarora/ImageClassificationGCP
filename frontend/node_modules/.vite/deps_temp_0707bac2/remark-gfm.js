...
while (code2 < codes5.leftCurlyBrace) {
  ...
  if (code2 === codes5.colon)
    code2 = codes5.uppercaseA;
  else if (code2 === codes5.leftSquareBracket)
    code2 = codes5.lowercaseA;
}
...
function tokenizeWwwPrefix(effects, ok5, nok) {
  ...
  function wwwPrefixAfter(code3) {
    return code3 === codes5.eof ? nok(code3) : ok5(code3);
  }
}
...
function tokenizeEmailDomainDotTrail(effects, ok5, nok) {
  ...
  function after(code3) {
    return asciiAlphanumeric3(code3) ? nok(code3) : ok5(code3);
  }
}
...
function tokenizeBlankLine(effects, ok5, nok) {
  ...
  function after(code3) {
    return code3 === codes6.eof || markdownLineEnding(code3) ? ok5(code3) : nok(code3);
  }
}
...
function tokenizeLabelStartImage(effects, ok5, nok) {
  ...
  function after(code3) {
    return code3 === codes6.caret && "_hiddenFootnoteSupport" in self.parser.constructs ? nok(code3) : ok5(code3);
  }
}
...
function tokenizeIndent(effects, ok5, nok) {
  ...
  function afterPrefix(code3) {
    ...
    return tail && tail[1].type === "gfmFootnoteDefinitionIndent" && tail[2].sliceSerialize(tail[1], true).length === constants6.tabSize ? ok5(code3) : nok(code3);
  }
}
...
function factorySpace2(effects, ok5, type, max) {
  const limit = max ? max - 1 : Number.POSITIVE_INFINITY;
  let size = 0;
  return start;
  function start(code3) {
...
function factorySpace3(effects, ok5, type, max) {
  const limit = max ? max - 1 : Number.POSITIVE_INFINITY;
  let size = 0;
  return start;
  function start(code3) {
...
function spaceThenNonSpace(effects, ok5, nok) {
  ...
  function after(code3) {
    return code3 === codes9.eof ? nok(code3) : ok5(code3);
  }
}
